...
func _ready() -> void:
	interaction_component.interact = Callable(self, "_on_open")

func _on_open(interactor: Player) -> void:
	if controlling:
		return
	player = interactor
...
func set_elevator_control_state(controlled) -> void:
	...
	player.set_player_input(controlled)
	if controlled: CameraTransition.transition_camera3D(player.camera_controller.player_camera, elevator_camera, 0.4)
	else: CameraTransition.transition_camera3D(elevator_camera, player.camera_controller.player_camera, 0.2)

func _process(delta: float) -> void:
	if controlling and player.get_movement_dir() != Vector3.ZERO:
